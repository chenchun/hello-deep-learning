...
class Generator(nn.Module):
  ...
  def __init__(self, z_dim=10, im_chan=3, hidden_dim=64):
    ...
    self.gen = nn.Sequential(self.make_gen_block(z_dim, hidden_dim * 8),
      self.make_gen_block(hidden_dim * 8, hidden_dim * 4), self.make_gen_block(hidden_dim * 4, hidden_dim * 2),
...
class Classifier(nn.Module):
  ...
  def __init__(self, im_chan=3, n_classes=2, hidden_dim=64):
    ...
    self.classifier = nn.Sequential(self.make_classifier_block(im_chan, hidden_dim),
...
def train_classifier(filename):
  ...
  dataloader = DataLoader(CelebA(".", split='train', download=True, transform=transform), batch_size=batch_size,
    shuffle=True)
